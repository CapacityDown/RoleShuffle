...
1. Tank and Runner use the greater of their configured minimum and Base Upgrade plus their configured bonus, capped at 200. Runner remains eligible if either of its two upgrades provides a benefit. Lifter treats the same formula as a requested limit, then selects the highest level at or below it that does not weaken normal grip for either vanilla weight category. If there is no safe increase, Lifter keeps Base and is excluded from normal role assignment. Superbot and copied Lifter abilities inherit this rule.
...
### Lifter Strength correction — 2026-09-17, build 461
...
- The requested level remains `max(configured role level, Base + bonus)`, capped at 200. Search downward from this limit to find a level above Base whose corrected translation and rotation are no worse in either weight category and better in at least one. Otherwise preserve Base; never reduce raw Strength, preserving rotation input too. Vanilla physics and temporary object/player overrides are unchanged.
- Default examples: Base 15 selects 19 instead of 25; Base 20 and 50 remain unchanged and exclude Lifter; Base 90 selects 95. Manual/draw adjustments are already included in Base before this comparison. The rule applies to overhaul-mode Lifter, copied Lifter and Superbot Strength; legacy mode and other role targets are unchanged.
- OverhaulChecks: 23,182 checks, including all Base levels 0–200 across 25 role-target/bonus combinations. Production eligibility/target checks: 10,349, including light/heavy weakening, safe partial gains, growth beyond the declining region, cap, Superbot, legacy behavior and a high target that improves translation but weakens rotation. In-game carry feel remains unverified.
- Build 461: zero warnings/errors. LocalizationChecks: 11,239 checks across 14 languages. Added missing glyphs to the Korean and both Chinese font subsets; Japanese coverage verified. Updated package and ZIP, preserving the original StageRoles artifacts.
- Verified 437 compiled game field references and deployed the five matching package files to RSO_TEST. Prior builds and the profile configuration are backed up; configuration remains unchanged and loading diagnostics remain disabled. Build 461 has not been launched for live carrying/rotation checks.
