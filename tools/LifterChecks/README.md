...
Coverage: upgrade levels 0–200, mass boundary 2, level-1 light handling versus the maximum heavy grip/rotation across levels 0–200 (143/24 at level 50), unchanged non-Lifter grabbers sharing an object, repeated calls without compounding, native level preservation, authority, stage cleanup, death, role changes, copied Lifter/Superbot capability, disabled mode, tumbling and explicit overrides. Only Base level 50 reaches the heavy target and is excluded from random assignment. A changed/unsupported method shape leaves the original instructions intact and disables random Lifter selection.
