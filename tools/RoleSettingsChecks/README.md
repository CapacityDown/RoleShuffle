...
Level adjustment checks compile the production level-entry mappings, adjustment service and Base Upgrades page callbacks. They cover all 12 existing REPOConfig string entries, one-level increments/decrements, preserving earlier/later run-level rules, blank and duplicate rules, numeric clamping, malformed input preservation, 0/200 and Map 0/1 limits, real save/reload and failed-save rollback, both autosave modes, stale run-level/host callbacks, config-to-UI refresh and guest host-value display. RoleCatalog's level snapshots, Unity rendering and transport remain stand-ins; the stored expressions and native BepInEx values are checked directly.
