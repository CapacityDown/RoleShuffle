...
- Lifter displays Strength 200 and fixes normal grip/rotation coefficients at six times vanilla level-1 values through host physics. All Base levels 0–200 remain eligible. Temporary overrides retain priority; copies and Superbot inherit the ability. Role changes and stage cleanup restore the target Strength level and cached grab force together.
- Added Jobless delivery contracts: carry different positive-value valuables at least 5 m outside the truck and bring them inside. Each of up to three contracts fully restores HP and grants a 30-second break from attrition. Stage start grants the same grace period; completed contracts survive revival and rejoining.
...
- Added a personal resource HUD below stamina for healing, revives, repair, charge, wagers and contracts. Native typography, cyan ability symbols and unit-free numbers appear in one vertical column, shrinking when necessary. Zero remains visible in red; fractional amounts round up. Copies and Superbot are supported, with independent local position, scale and visibility settings.
