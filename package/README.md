...
| Key | Default | Range / values | Effect | Control |
|---|---:|---|---|---|
| `Base Upgrades.ManualAdjustmentEnabled` | `false` | `true`, `false` | Enables manual adjustments and the minus/plus buttons. Off hides the buttons and the Manual breakdown and excludes saved manual amounts from base targets; re-enabling restores them. Base rules and truck draws are unaffected. | Host |
...
| キー | デフォルト | 範囲・値 | 内容 | 管理 |
|---|---:|---|---|---|
| `Base Upgrades.ManualAdjustmentEnabled` | `false` | `true`、`false` | 手動調整と±ボタンを有効にします。OFFではボタンと内訳の「手動」を非表示にし、保存済み調整値を基礎値から除外します。再びONにすると復元します。設定式とトラック抽選には影響しません。 | ホスト |
