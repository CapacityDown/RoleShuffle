...
internal sealed class RoleBugReport : ILogListener
{
    ...
    public void LogEvent(object sender, LogEventArgs args)
    {
        ...
        lock (_gate)
        {
            _logs.Enqueue(value.Substring(0, Math.Min(value.Length, 2048)));
            while (_logs.Count > 100) _logs.Dequeue();
        }
    }
    ...
    internal void Create(ConfigFile config)
    {
        ...
        report.AppendLine("\n## Recent RoleShuffle logs (up to 100 entries, newest first)\n");
        string[] privateValues;
        lock (_gate)
        {
            sectionStart = report.Length;
            foreach (string log in _logs.Reverse())
            {
                if (report.Length - sectionStart + log.Length > 24000) { report.AppendLine("[TRUNCATED]"); break; }
